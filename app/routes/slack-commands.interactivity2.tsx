...
async function sendSlackMessagesAllTasks(
  ...
  try {
    for (let i = 0; i < resultTasks.rows.length; i++) {
      ...
      await sendSlackMessageButton( token,
        userSlackId,
        projectName,
        task.title,
        task.priority,
      ...
      blocks: [
        {
          ...
          elements: [
            ...
            {
              type: "image",
              image_url: "https://em-content.zobj.net/source/animated-noto-color-emoji/356/dog_1f415.gif",
              alt_text: "cute cat",
            },
          ],
        },
      ...
      blocks: [
        {
          ...
          elements: [
            {
              type: "button",
              text: { type: "plain_text", text: `Update Status For Task ${taskNo}` },
              value: "done",
            ...
            {
              type: "button",
              text: { type: "plain_text", text: "Awaiting-Triage" },
              value: `${task}`,
              ...
              confirm: {
                ...
                "text": {
                  "type": "mrkdwn",
                  "text": `Are you sure you want to update the status for task ${taskNo} to Awaitingh-Triage `
                },
                "confirm": {
                  "type": "plain_text",
                  "text": "Yes"
          ...
          elements: [
            {
              ...
              elements: [
                {
                  type: "text",
                  text: ` ${srNO}${srNoSpaces} ${project}${projectSpaces}${truncatedTask}${taskSpaces}${priorityEmogi}${toTitleCase(priority)}${prioritySpaces}      ${statusEmogi} ${toTitleCase(statuss)}${statussSpaces}`,
                },
              ],
            },
...
export const action: ActionFunction = async ({ request }) => {
  try {
    ...
    if (payload.type === "view_submission") {
      ...
      if (payload.callback_id == "delete_chat") {
        ...
        console.log(payload);
        console.log("delete-chat chala")

       const channel='D06NLH8JL64';

       const token=getRequiredEnvironmentVariable("SLACK_AUTH_TOKEN")
        const apiUrl = "https://slack.com/api/conversations.history";
        ...
        if (!responseData.ok) {
          throw new Error(`Failed to retrieve conversation history: ${responseData.error}`);
        }
