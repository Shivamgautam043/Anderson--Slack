...
const spaceCreater = (str: string, size: number) => {
    let spaces = "";
...
async function sendSlackMessagesAllTasks(
    ...
    try {
        for (let i = 0; i < resultTasks.rows.length; i++) {
            ...
            await sendSlackMessageButton(token, userSlackId, task.title, i + 1);
        }
    } catch (error) {
                    ...
                    elements: [
                        {
                            ...
                            elements: [
                                {
                                    type: "text",
                                    text: `Sr.No.  ${project}${spaceCreater(
                                        project,
                                        40
                                    )}${task}${spaceCreater(
                                        task,
                                        50
                                    )}${priority}${spaceCreater(
                                        priority,
                                        10
                                    )}         ${statuss}${spaceCreater(
                                        statuss,
                                        20
                                    )}`,
                                },
...
async function sendSlackMessageButton(
    token: string,
    channel: string,
    task: string,
    taskNo: number
                    ...
                    elements: [
                        {
                            ...
                            text: {
                                type: "plain_text",
                                text: `Update Status For Task ${taskNo}`,
                            },
                            value: "done",
                            action_id: "status_input_action0",
                        },
                        {
                            type: "button",
                            text: { type: "plain_text", text: "Done" },
                            value: `${task}`,
                            action_id: "status_input_action1",
                            confirm: {
                                title: {
                                    type: "plain_text",
                                    text: "Are you sure?",
                                ...
                                text: {
                                    type: "mrkdwn",
                                    text: `Are you sure you want to update the status for task ${taskNo} to Done`,
                                },
                                confirm: {
                                    type: "plain_text",
                                    text: "Yes",
                        ...
                        },
                        {
                            ...
                            value: `${task}`,
                            action_id: "status_input_action2",
                            confirm: {
                                title: {
                                    type: "plain_text",
                                    text: "Are you sure?",
                                ...
                                text: {
                                    type: "mrkdwn",
                                    text: `Are you sure you want to update the status for task ${taskNo} to To-Do`,
                                },
                                confirm: {
                                    type: "plain_text",
                                    text: "Yes",
                        ...
                        },
                        {
                            ...
                            value: `${task}`,
                            action_id: "status_input_action3",
                            confirm: {
                                title: {
                                    type: "plain_text",
                                    text: "Are you sure?",
                                ...
                                text: {
                                    type: "mrkdwn",
                                    text: `Are you sure you want to update the status for task ${taskNo} to In-Progress`,
                                },
                                confirm: {
                                    type: "plain_text",
                                    text: "Yes",
                        ...
                        },
                        {
                            ...
                            value: `${task}`,
                            action_id: "status_input_action4",
                            confirm: {
                                title: {
                                    type: "plain_text",
                                    text: "Are you sure?",
                                ...
                                text: {
                                    type: "mrkdwn",
                                    text: `Are you sure you want to update the status for task ${taskNo} to Awaiting-Triage `,
                                },
                                confirm: {
                                    type: "plain_text",
                                    text: "Yes",
...
async function sendSlackMessage(
    ...
    const truncatedProject = truncateString(project, 35);
    try {
        const message = {
            channel: channel,
            blocks: [
                    ...
                    elements: [
                        {
                            ...
                            elements: [
                                {
                                    type: "text",
                                    text: ` ${srNO}${spaceCreater(
                                        srNO.toString(),
                                        6
                                    )} ${truncatedProject}${spaceCreater(
                                        project,
                                        40
                                    )}${truncatedTask}${spaceCreater(
                                        truncatedTask,
                                        50
                                    )}${priorityEmogi}${toTitleCase(
                                        priority
                                    )}${spaceCreater(
                                        priority,
                                        10
                                    )}      ${statusEmogi} ${toTitleCase(
                                        statuss
                                    )}${spaceCreater(statuss, 6)}`,
                                },
                            ],
                        },
...
export const action: ActionFunction = async ({ request }) => {
    try {
        ...
        if (payload.type === "view_submission") {
            console.log("modal submission run");
            const userId =
                payload.view.state.values.user_input.user_input_action
            ...
            // this is the user id who is interacting
            ...
            const token: string =
                getRequiredEnvironmentVariable("SLACK_AUTH_TOKEN");

            const postgresDatabaseManager = await getPostgresDatabaseManager(
                null
            );
            if (postgresDatabaseManager instanceof Error) {
                throw new Error("Error connecting to database");
      ...
      `,
            ...
            console.log("block action run");
            // console.log(payload)
            console.log(payload.actions[0].text);

            if (payload.actions[0].block_id == "status_input") {
                console.log("status input chla");
                const taskName = payload.actions[0].value;
                const taskStatus = payload.actions[0].text.text.toUpperCase();
                console.log(taskStatus);
                console.log(taskName);

                const postgresDatabaseManager =
                    await getPostgresDatabaseManager(null);
                if (postgresDatabaseManager instanceof Error) {
                    throw new Error("Error connecting to database");
                }
          ...
          `,
                ...
                console.log(result);
                // console.log("updation chla")
                console.log("update button run");
            }
